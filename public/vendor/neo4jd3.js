...
function Neo4jD3(_selector, _options) {
    ...
    function appendTextToNode(node) {
        return node.append('text')
                   ...
                   .attr('font-size', function(d) {
                       return icon(d) ? (Math.round(options.nodeRadius * 0.55) + 'px') : '10px';
                   })
                   .attr('pointer-events', 'none')
                   .attr('text-anchor', 'middle')
                   .attr('y', function(d) {
                       return icon(d) ? (Math.round(options.nodeRadius * -0.1) + 'px') : '4px';
                   })
                   .html(function(d) {
                       var _icon = icon(d);
    ...
    function appendLabelToNode(node) {
        return node.append('text')
                   ...
                   .attr('fill', '#ffffff')
                   .attr('font-size', Math.max(8, Math.round(options.nodeRadius * 0.22)) + 'px')
                   .attr('pointer-events', 'none')
                   .attr('text-anchor', 'middle')
                   .attr('y', function(d) {
                       return Math.round(options.nodeRadius * 0.45) + 'px';
                   })
                   .text(function(d) {
                       var label = (d.properties && d.properties.name) ? d.properties.name : d.id;
                       var max = Math.floor(options.nodeRadius / 4.5);
...
*/
    ...
    function updateNodes(n) {
        ...
        for (var i = 0; i < nodes.length; i++) {
            existingIds[nodes[i].id] = true;
        }
        for (var j = 0; j < n.length; j++) {
            if (!existingIds[n[j].id]) {
                nodes.push(n[j]);
            }
        }
    ...
    function updateRelationships(r) {
        ...
        for (var i = 0; i < relationships.length; i++) {
            existingIds[relationships[i].id] = true;
        }
        for (var j = 0; j < r.length; j++) {
            if (!existingIds[r[j].id]) {
                relationships.push(r[j]);
            }
        }
