...
describe('binary-manager', () => {
  ...
  describe('updateBinary', () => {
    it('removes old binary dir, re-downloads, and clears caches', async () => {
      setPlatform('darwin', 'arm64');
      execFileSyncFn = (cmd: string) => {
        if (cmd === 'tar') return '';
        throw new Error('not found');
      ...
      await updateBinary('/tmp/base');
      assert.ok(rmCalled, 'fs.rm should have been called to remove old binary');
    });
  });
  ...
  describe('checkForUpdate', () => {
    it('returns available:true when remote is newer', async () => {
      ...
      spawnSyncFn = () => ({
        stdout: '', stderr: 'version: 8200 (abc)\n', status: 0,
      });
      ...
      const info = await checkForUpdate('/tmp/base');
      assert.equal(info.available, true);
      assert.equal(info.currentBuild, 8200);
      assert.equal(info.latestBuild, 8300);
      assert.equal(info.latestTag, 'b8300');
      assert.ok(info.daysNewer !== null && info.daysNewer >= 0);
    });

    it('returns available:false when local is up to date', async () => {
      setPlatform('darwin', 'arm64');
      execFileSyncFn = (cmd: string) => {
        if (cmd === 'which') return '/usr/local/bin/llama-server\n';
        throw new Error('not found');
      ...
      assert.equal(info.currentBuild, 8300);
      assert.equal(info.latestBuild, 8300);
    });

    it('returns available:false when no local version', async () => {
      setPlatform('darwin', 'arm64');
      execFileSyncFn = () => { throw new Error('not on PATH'); };
      existsSyncFn = () => false;
      ...
      const info = await checkForUpdate('/tmp/base');
      assert.equal(info.available, false);
      assert.equal(info.currentBuild, null);
    });
    ...
    it('handles GitHub API failure gracefully', async () => {
      ...
      assert.equal(info.available, false);
      assert.equal(info.currentBuild, 8200);
      assert.equal(info.latestBuild, null);
    });
    ...
    it('handles missing tag_name gracefully', async () => {
      ...
      fetchFn = async () => ({
        ok: true,
        json: async () => ({ published_at: '2025-01-01T00:00:00Z' }),
      });
    ...
    it('daysNewer is null when local is newer or same', async () => {
      setPlatform('darwin', 'arm64');
      execFileSyncFn = (cmd: string) => {
        if (cmd === 'which') return '/usr/local/bin/llama-server\n';
        throw new Error('not found');
      ...
      const info = await checkForUpdate('/tmp/base');
      assert.equal(info.daysNewer, null);
    });
  });
});
